...
**rclone wrapper**: `rclone.New()` falls back to common install dirs (Homebrew, `~/.local/bin`, WinGet/Scoop shims) when rclone is not on PATH, because cron runs with a minimal PATH. List-style commands use `output()` (capture stdout, stderr → error). Long operations use `stream()` — one `os.Pipe` receives stdout+stderr and a custom `bufio.SplitFunc` (`scanLinesOrCR`) splits on `\n` **and** `\r`, so `-P` progress updates surface live. All calls take a `context.Context`.
...
- **Clean only deletes inside the backup folders Upload writes** (`backup.CleanTargets`: `<dest>/<basename>` per source folder) — never at the destination itself, which may be the account root holding unrelated files. It refuses `RemoteRetentionDays < 1`.
- **Clean enforces a per-folder safety lock**: before deleting from a backup folder it confirms that folder holds a file newer than `RemoteCleanupSafetyDays` (`rclone lsf --max-age`); a folder without one is skipped and Clean returns `ErrNoRecentBackup` after processing the rest. `CleanOptions.Force` bypasses it — treat as dangerous. (`backup/clean.go`)
